...
## Investigation Brief
Build Minutes climbs when commits rebuild unchanged work. Common causes: `TURBO_FORCE`, `cache: false`, missing Turbo outputs, or disabled build-skip settings.

## Evidence To Check
Confirm Build Minutes share and scanner subtype. Inspect `package.json`, `turbo.json`, generated outputs, `.gitignore`, `vercel.json`, and project-settings evidence.
...
## Verification
Name the offending file and pattern. Recommend only the verified fix: re-enable cache, add generated `outputs`, enable skip-unaffected builds, or add `ignoreCommand` only when needed.
